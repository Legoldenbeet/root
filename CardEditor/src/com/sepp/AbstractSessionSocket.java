...
public abstract class AbstractSessionSocket implements Runnable {
	/**
	 * 所有会话列表
	 */
	private static ArrayList<AbstractSessionSocket> sessiontList = new ArrayList<AbstractSessionSocket>();
	 ...
	 */
	public AbstractSessionSocket(Socket socket) {
		this.socket = socket;
		registSession(this);
	 ...
	 */
	public synchronized ArrayList<AbstractSessionSocket> getSessions() {
	 ...
	 * @return 返回类型 void
	 */
	public synchronized void registSession(AbstractSessionSocket session) {
		if (!REGIST_DISABLED) {
	 ...
	 * @return 返回类型 void
	 */
	public synchronized void unregistSession(AbstractSessionSocket session) {
		if (!REGIST_DISABLED) {
			for (int i = 0; i < sessiontList.size(); i++) {
				AbstractSessionSocket session2 = sessiontList.get(i);
